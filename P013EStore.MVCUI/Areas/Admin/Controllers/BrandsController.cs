...
namespace P013EStore.MVCUI.Areas.Admin.Controllers
{
    ...
    public class BrandsController : Controller
    {
        ...
        {
            if (id == null)// id göndermeden edit sayfası açılmaya çalışıldığında 
            {
                return BadRequest();// geriye geçersiz istek hatası dön
            }
            var model = await _service.FindAsync(id.Value);// nullable yapınca yukarda ? koyarak value koyarak kesin bir değer almış gibi oluyoruz bizim metodumuz mutlaka id istediği için
            if (model == null)
            {
