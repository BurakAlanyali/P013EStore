...
namespace P013EStore.Data.Concrete
{
	public class ProductRepository : Repository<Product>, IProductRepository
	{
		...
		{
			return await _context.Products.Include(x => x.Category).Include(x => x.Brand).FirstOrDefaultAsync(x => x.Id == id);
		}
		...
		{
			return await _context.Products.Include(x=>x.Category).Include(x=>x.Brand).Where(expression).ToListAsync();
		}
	}
}
